...
DriveType=table(actdata$venue,actdata$Pct_Drive)
#Compute marginal percents
(Drivetype_mgCol=prop.table(DriveType,
                            margin = 2)%>%round(.,1))
#Generate a data frame to pull plot data from
driveframe=as.data.frame(DriveType)
names(driveframe)=c("Venue","Percent Drivers")

#Cut code return here
driveframe$pctCol=as.data.frame(Drivetype_mgCol)[,2]
